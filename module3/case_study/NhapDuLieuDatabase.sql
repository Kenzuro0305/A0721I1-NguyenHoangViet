...
-- câu 5. 5.	Hiển thị ma_khach_hang, ho_ten, ten_loai_khach, ma_hop_dong, ten_dich_vu, ngay_lam_hop_dong, ngay_ket_thuc, tong_tien (Với tổng tiền được tính theo công thức như sau: Chi Phí Thuê + Số Lượng * Giá, với Số Lượng và Giá là từ bảng dich_vu_di_kem, hop_dong_chi_tiet) cho tất cả các khách hàng đã từng đặt phòng. (những khách hàng nào chưa từng đặt phòng cũng phải hiển thị ra).
select khach_hang.ma_khach_hang, khach_hang.ho_ten, loai_khach.ten_loai_khach, hop_dong.ma_hop_dong, dich_vu.ten_dich_vu, hop_dong.ngay_lam_hop_dong, hop_dong.ngay_ket_thuc, 
 ...
 where not exists (select hop_dong.ma_hop_dong from hop_dong where (hop_dong.ngay_lam_hop_dong between "2021-01-01" and "2021-03-31") and hop_dong.ma_dich_vu = dich_vu.ma_dich_vu);
...
select month(hop_dong.ngay_lam_hop_dong) as thang, count(month(hop_dong.ngay_lam_hop_dong)) as so_khach_hang from hop_dong where year(hop_dong.ngay_lam_hop_dong) ="2021"
...
where not exists(select hop_dong.ma_hop_dong from hop_dong where (hop_dong.ngay_lam_hop_dong between "2021-01-01" and "2021-06-31") and hop_dong.ma_hop_dong = hop_dong_chi_tiet.ma_hop_dong)
and exists (select hop_dong.ma_hop_dong from hop_dong where (hop_dong.ngay_lam_hop_dong between "2020-10-01" and "2020-12-31") and hop_dong.ma_hop_dong = hop_dong_chi_tiet.ma_hop_dong)
group by hop_dong.ma_hop_dong;